...
class lessc {
	...
	public static function ccompile($in, $out = null) {
		if(!$out) $out = str_replace('.less', '.css', $in);
		
		if (!is_file($out) || filemtime($in) > filemtime($out)) {
			$less = new lessc($in);
			file_put_contents($out, CssMin::minify($less->parse()));
		}
		
		$output = new stdClass();
		$output->real_path = $out;
		$output->relative_path = str_replace(root, '~/', $out);
		return $output;
	}
