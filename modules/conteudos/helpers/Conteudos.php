    ...
    public static function cadastrar($propriedades) {
        ...
        foreach ($pagina_properties as $k) {
            if (isset($propriedades[$k])) {
                if (isset(self::$validation[$k])) {
                    if(isset(self::$validation[$k]['regExp']) && preg_match(self::$validation[$k]['regExp'], $propriedades[$k]) === 0) {
                        throw new ValidationException('Formato de ' . $k . ' inválido.');
                    } elseif(isset(self::$validation[$k]['length'])) {
                        $len = strlen($propriedades[$k]);
                        if($len <= self::$validation[$k]['length'][0] || $len > self::$validation[$k]['length'][1]){
                            throw new ValidationException('Formato de ' . $k . ' inválido.');
                        }else{
